...
root = Tk()#defining the window and storing in the variable root
...
def openFile():
    try:
        root.songAdded = True
        root.filename = filedialog.askopenfilename(initialdir = "/",title = "Select your cool music track",
...
def contact():
    ...
    webbrowser.open("https://www.facebook.com/kumar.aryan.58958343")
    
def contribute():
    webbrowser.open("https://github.com/ram016/Music-Player")
...
one = Label(root, text="Welcome", font=('Algerian', 12,'bold'),bg="Black",fg="Red")
one.pack(fill=X)
two = Label(root, text="To RKP Music Player", bg="white",fg="blue")
two.pack(fill=X)
three = Label(root, text="Created By Ram Kumar Pandey", font=('Times New Roman',12,'bold'),bg="purple",fg="white")
three.pack(fill=X)
...
root.screenMessage.set("Welcome, to RKP Music Player")
